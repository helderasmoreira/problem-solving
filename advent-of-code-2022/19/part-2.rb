...
results = blueprints.take(3).map do |blueprint|
  puts blueprint
  ...
  while queue.any?
    ...
    next if current.materials[:geode] + (current.robots[:geode] * current.remaining_time) + posssible_geodes_for_remaining_time(current.remaining_time - 1) <= best_geode_state.materials[:geode]
...
puts results.map { |r| result.materials[:geode] }.inject(:*)
