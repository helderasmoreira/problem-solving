...
def unexplored_edges_for(current:, map:)
  edges = [
...
starts = map.flatten.select { |node| node.value == 'a' }
starts.each { |start| start.explored = true }
queue.push(*starts)
...
while current = queue.shift do
  if current.value == 'E'
    ...
    unexplored_edges_for(current: current, map: map).each do |node|
      node.explored = true
